...
COMBINED_PATTERN_STR = "|".join([
    f"(?:{DECIMAL_PATTERN.pattern})",
    f"(?:{DMS_PATTERN.pattern})",
    f"(?:{DDM_PATTERN.pattern})",
    f"(?:{DD_COMPASS_PATTERN.pattern})",
    f"(?:{UTM_PATTERN.pattern})",
    f"(?:{GOOGLE_MAPS_PATTERN.pattern})",
    f"(?:{APPLE_MAPS_PATTERN.pattern})",
    f"(?:{PLUS_CODE_PATTERN.pattern})",
    f"(?:{W3W_PATTERN.pattern})"
])
...
def load_data(files):
    ...
    # --- Date Fallback Logic (Vectorized) ---
    fallback_df = pd.DataFrame({
        'year': df['Year'],
        'month': df['Month'].fillna(1),
        'day': df['Day'].fillna(1)
    })
    fallback_dates = pd.to_datetime(fallback_df, errors='coerce')
    ...
    # Sanitize backticks to prevent JavaScript crashes (regex=False for speed)
    df['Locality'] = df['Locality'].str.replace('`', "'", regex=False)
    df['Observation Details'] = df['Observation Details'].str.replace('`', "'", regex=False)
    ...
    df['group_key'] = df['eBird Checklist ID'].fillna(df['ML Catalog Number'])
    
    # --- Vectorized Emoji Logic & Flagging ---
    dummy_df = pd.DataFrame({
        'group_key': df['group_key'],
        'Photo': df['Format'] == 'Photo',
        'Video': df['Format'] == 'Video',
        'Audio': df['Format'] == 'Audio'
    })
    format_flags = dummy_df.groupby('group_key').max()
    
    emoji_arr = np.where(format_flags['Photo'], "📷 ", "") + \
                np.where(format_flags['Video'], "📹 ", "") + \
                np.where(format_flags['Audio'], "🔊 ", "")
                
    emoji_series = pd.Series(emoji_arr, index=format_flags.index).str.strip()
    df['Media_Emojis'] = df['group_key'].map(emoji_series)

    # Separate NaNs, drop duplicates, combine back
    ...
    # Merge media flags directly into the unique dataframe for instant boolean map filtering
    df_unique = df_unique.merge(format_flags, on='group_key', how='left')

    # Pre-compute Time formatting (Moves heavy Python string operations out of the Map render loop)
    time_numeric = pd.to_numeric(df_unique['Time'], errors='coerce').fillna(0).astype(int)
    time_str = time_numeric.astype(str).str.zfill(4)
    df_unique['formatted_time'] = time_str.str[:-2] + ':' + time_str.str[-2:]

    # Pre-compute eBird URL properties
    is_ebird = df_unique['eBird Checklist ID'].notna()
    df_unique['cl_url'] = np.where(
        is_ebird, 
        "https://ebird.org/checklist/" + df_unique['eBird Checklist ID'].astype(str), 
        "https://macaulaylibrary.org/asset/" + df_unique['ML Catalog Number'].astype(str)
    )
    df_unique['cl_txt'] = np.where(
        is_ebird, 
        df_unique['eBird Checklist ID'].astype(str), 
        "ML" + df_unique['ML Catalog Number'].astype(str)
    )
    ...
    # Vectorized Checkbox Map Optimization
    df_unique['icon_group'] = 'media_or_blank'
    
    valid_details = df_unique['Observation Details'].notna() & (df_unique['Observation Details'].str.strip() != "")
    
    has_coords_mask = df_unique['Observation Details'].str.contains(
    ...
    df_unique.loc[valid_details, 'icon_group'] = 'comment'
    df_unique.loc[valid_details & has_coords_mask, 'icon_group'] = 'compass'
...
if uploaded_files:
    try:
        ...
        if selected_localities:
            df_filtered = df_unique[df_unique["Locality"].isin(selected_localities)].copy()
...
# --- 5. Main Content Area ---
    ...
    with list_col:
        ...
        if not df_display.empty:
            ...
            def style_table(df):
                ...
                color_array = np.tile(colors[:, None], (1, len(df.columns)))
                return pd.DataFrame(color_array, index=df.index, columns=df.columns)
            ...
            st.dataframe(
                ...
                key="loc_table",
                column_config={
                    "Date": st.column_config.Column("Date", width="small", required=True),
                    "Checklists": st.column_config.Column("Checklists", width="small", required=True),
                    "Locality": st.column_config.Column("Locality", width="large")
                },
                width='stretch'
            )
        else:
    ...
    with chart_col2:
        ...
        timeline_data = (
            df_filtered.groupby(df_filtered['Date_obj'].dt.normalize())['eBird Checklist ID']
            .nunique()
            .reset_index()
        )
        ...
        year_title = f"Year ({min_year})" if min_year == max_year else f"Years ({min_year}–{max_year})"
    ...
    with chart_col1:
        ...
        # C-Optimized datetimes generation using dictionary unpacking (massively faster than dt.strftime strings)
        df_filtered['Seasonal_Date'] = pd.to_datetime({
            'year': 2000,
            'month': df_filtered['Date_obj'].dt.month,
            'day': df_filtered['Date_obj'].dt.day
        }, errors='coerce')
    ...
    with map_col:
        ...
        if not df_map.empty:
            if "🧭" in st.session_state.map_filters_pill:
                ...
                if not selected_media:
                    ...
                    # Massively faster boolean index masking replaces regex row-by-row emoji evaluations
                    mask = pd.Series(False, index=df_map.index)
                    if "🔊" in selected_media: mask |= df_map['Audio']
                    if "📹" in selected_media: mask |= df_map['Video']
                    if "📷" in selected_media: mask |= df_map['Photo']
                    df_map = df_map[mask]
        ...
        def render_map(data):
            ...
            for row in clean_df.itertuples(index=False):
                ...
                # Time strings and URLs are now precomputed efficiently outside the loop
